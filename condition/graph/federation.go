...
func (ec *executionContext) __resolve_entities(ctx context.Context, representations []map[string]interface{}) []fedruntime.Entity {
	...
	resolveEntity := func(ctx context.Context, typeName string, rep map[string]interface{}, idx []int, i int) (err error) {
		...
		switch typeName {
		case "Condition":
			resolverName, err := entityResolverNameForCondition(ctx, rep)
			if err != nil {
				return fmt.Errorf(`finding resolver for Entity "Condition": %w`, err)
			}
			switch resolverName {
			...
			case "findConditionByID":
				...
				if err != nil {
					return fmt.Errorf(`unmarshalling param 0 for findConditionByID(): %w`, err)
				}
				entity, err := ec.resolvers.Entity().FindConditionByID(ctx, id0)
				if err != nil {
					return fmt.Errorf(`resolving Entity "Condition": %w`, err)
				}
...
func entityResolverNameForCondition(ctx context.Context, rep map[string]interface{}) (string, error) {
	for {
		var (
		...
		return "findConditionByID", nil
	}
	return "", fmt.Errorf("%w for Condition", ErrTypeNotFound)
}
